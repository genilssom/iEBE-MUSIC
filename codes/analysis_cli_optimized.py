        ...
        if block_type == 'p':
            ...
            for _ in range(n_part_lines):
                ...
                particles.append([
                    ...
                    unpacked_data[7],  # py
                    unpacked_data[8],  # pz
                    unpacked_data[9],  # pid (pdg)
                    unpacked_data[11], # charge (index 11; 10 is particle ID)
...
def main():
    ...
    if args.mode == '2D':
        ...
        print("Stacking all events into a single array...")
        all_particles = np.concatenate(events)  # shape (N_total, 7): E,px,py,pz,pid,charge,mass
        ...
        py      = all_particles[:, 2]
        pz      = all_particles[:, 3]
        pid_arr = all_particles[:, 4].astype(np.int32)
